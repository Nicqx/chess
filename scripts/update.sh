...
IMAGE="chess-game:${REV}-${NODE_ARCH}"
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT INT TERM
sed "s|image: chess-game:1.1.0|image: $IMAGE|" chess-game-deployment.yaml > "$TMP/resources.yaml"
...
STATE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/nicqx-apps/$TARGET/chess-game"
mkdir -p -m 700 "$STATE_DIR"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
$KUBECTL get deployment/chess-game-deployment service/chess-game-service -n "$NAMESPACE" -o yaml \
...
$KUBECTL rollout status deployment/chess-game-deployment -n "$NAMESPACE" --timeout=180s
echo "Ready: Deployment/chess-game-deployment ($IMAGE)"
